...
def transfers(k, archetype, influence, targets, public, cfg):
    ...
    # A Commerce player must put at least one chip into the market before claiming
    # a net profit; after that, the social model determines whether others pay them.
    commerce_paid = sum(k.given_total.values())
    if k.motive == COMMERCE and commerce_paid < cfg.commerce_min_paid:
        trade = max(trade, 0.9)
